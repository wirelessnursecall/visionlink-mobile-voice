...
class CoreContext @UiThread constructor(val context: Context) : HandlerThread("Core Thread") {
    ...
    fun onCoreStarted() {
        ...
        if (oldVersion < currentVersion) {
            ...
            if (oldVersion < 100000) { // 1.0.0 initial release
        ...
        notificationsManager.onCoreStarted(core, oldVersion < 100000) // Re-create channels when migrating from a non 6.0 version
    ...
    fun startVideoCall(
        ...
        val params = core.createCallParams(null)
        params?.isVideoEnabled = false // true
    ...
    fun startCall(
        ...
        // this ok for transfer ip
        params.addCustomHeader("X-extraData", LinphoneUtils.getLocalIpAddressIpv4())
