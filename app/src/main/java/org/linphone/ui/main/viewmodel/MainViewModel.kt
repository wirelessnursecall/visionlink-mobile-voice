...
class MainViewModel @UiThread constructor() : ViewModel() {
    ...
    val AccountRegistrationErrorEvent: MutableLiveData<Event<Boolean>> by lazy {
        MutableLiveData<Event<Boolean>>()
    }

    val AccountRegistrationSuccessEvent: MutableLiveData<Event<Boolean>> by lazy {
        MutableLiveData<Event<Boolean>>()
    }

    val AccountRegistrationInProgressEvent: MutableLiveData<Event<Boolean>> by lazy {
        MutableLiveData<Event<Boolean>>()
    }
            ...
            when (state) {
                RegistrationState.Failed -> {
                    if (account == core.defaultAccount) {
                        ...
                        AccountRegistrationErrorEvent.postValue(Event(true))
                    }
                }
                RegistrationState.Ok -> {
                    removeAlert(NETWORK_NOT_REACHABLE) // Just in case
                    removeAlert(NON_DEFAULT_ACCOUNT_NOT_CONNECTED) // remove alert if connected is ok
                    ...
                    AccountRegistrationSuccessEvent.postValue(Event(true))
                }
                RegistrationState.Progress, RegistrationState.Refreshing -> {
                    ...
                    AccountRegistrationInProgressEvent.postValue(Event(true))
                }
                else -> {}
            }
        }
